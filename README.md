...
本アプリケーションは、大学の「知能情報実験実習2」の実装課題として作成した、**堅牢なセキュリティ設計**を特徴とするWebアプリケーションです。Next.js (App Router) を用いて、モダンな**トークンベース認証（JWT）**と、それに関連する高度なセキュリティ機能をゼロから実装しました。
...
（**[推奨]** ここにGIFアニメーションまたは動画を配置すると、評価が格段に向上します）

![パスワードリセットのデモGIF](./docs/images/Animation.gif)
*（キャプション例）パスワードリセット要求から、Mailtrapでメールを受信し、パスワードを再設定するまでの一連の流れ。*
